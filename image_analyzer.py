...
class AdvancedImageAnalyzer:
    ...
    # Эти методы вызываются, но не были реализованы.
    # Они должны быть ВНУТРИ класса AdvancedImageAnalyzer
    ...
    def _calculate_composition_complexity_advanced(self) -> float:
        ...
        if CV2_AVAILABLE and self.image_gray is not None:
            edges = cv2.Canny(self.image_gray, 50, 150)
            complexity = np.sum(edges) / (edges.size * 255)
            return min(complexity * 5, 1.0)
        return 0.5
